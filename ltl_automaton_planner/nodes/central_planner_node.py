...
class Central_Planner(object):
    ...
    def init_ts_state_from_agent(self, msg=TransitionSystemStateStamped):
        ...
        if (len(msg.ts_state.states) == len(msg.ts_state.state_dimension_names)):
            ...
            rospy.logerr("LTL Global Planner: received initial states don't match TS state models: "+str(len(msg.ts_state.states))+" initial states and "+str(len(msg.ts_state.state_dimension_names))+" state models")
    ...
    def build_automaton(self):
        ...
        # Initial allocation
        self.ltl_planner_multi_robot = GlobalLTLPlanner(self.ts_list, self.hard_task, self.soft_task, self.initial_beta, self.gamma)
    ...
    def setup_pub_sub(self):
        ...
        self.plan_pub_1 = rospy.Publisher('/dr_0/global_planner/action_plan', LTLStateArray, latch=True, queue_size = 1)
        self.plan_pub_2 = rospy.Publisher('/a1_gazebo/global_planner/action_plan', LTLStateArray, latch=True, queue_size = 1)
        self.plan_pub_3 = rospy.Publisher('/wassi_0/global_planner/action_plan', LTLStateArray, latch=True, queue_size = 1)
    ...
    def ltl_replan_callback_1(self, msg):
        ...
        if(replan_status == 1):
            ...
            rospy.loginfo("LTL Global Planner: local replan rname is: %d" %self.ltl_planner_multi_robot.local_replan_rname)
            # else:
            #     rospy.logerr("LTL Global Planner: local replan rname is not empty")
        ...
        if(replan_status == 3):
            rospy.logwarn('LTL Global Planner: received replanning Level 3: handling transition system change from agent 1')
            # Replan
            start = time.time()
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
            self.ltl_planner_multi_robot.local_replan_rname = 0
    ...
    def ltl_replan_callback_2(self, msg):
        ...
        if(replan_status == 1):
            rospy.logwarn('LTL Global Planner: received replanning Level 1; handling malfunction from agent 2')
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
        ...
        if(replan_status == 2):
            rospy.logwarn('LTL Global Planner: received replanning Level 2: handling abrupt state change from agent 2')
            # Replan
            start = time.time()
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
            self.ltl_planner_multi_robot.local_replan_rname = 1
        ...
        if(replan_status == 3):
            rospy.logwarn('LTL Global Planner: received replanning Level 3: handling transition system change from agent 2')
            # Replan
            start = time.time()
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
            self.ltl_planner_multi_robot.local_replan_rname = 1
    ...
    def ltl_replan_callback_3(self, msg):
        ...
        if(replan_status == 1):
            rospy.logwarn('LTL Global Planner: received replanning Level 1; handling malfunction from agent 3')
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
        ...
        if(replan_status == 2):
            rospy.logwarn('LTL Global Planner: received replanning Level 2: handling abrupt state change from agent 3')
            # Replan
            start = time.time()
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
            self.ltl_planner_multi_robot.local_replan_rname = 2
        ...
        if(replan_status == 3):
            rospy.logwarn('LTL Global Planner: received replanning Level 3: handling transition system change from agent 3')
            # Replan
            start = time.time()
            # if self.ltl_planner_multi_robot.local_replan_rname is not None:
            self.ltl_planner_multi_robot.local_replan_rname = 2
    ...
    def ts_trace_callback_1(self, msg=LTLPlan()):
        ...
        if not (self.check_timestamp and (msg.header.stamp.to_sec() == self.prev_received_timestamp_1.to_sec())):
            ...
            rospy.logwarn("LTL Global Planner: not updating with received trace, timestamp identical to previously received message timestamp at time %f" %self.prev_received_timestamp_1.to_sec())
    ...
    def ts_trace_callback_2(self, msg=LTLPlan()):
        ...
        if not (self.check_timestamp and (msg.header.stamp.to_sec() == self.prev_received_timestamp_2.to_sec())):
            ...
            rospy.logwarn("LTL Global Planner: not updating with received trace, timestamp identical to previously received message timestamp at time %f" %self.prev_received_timestamp_2.to_sec())
    ...
    def ts_trace_callback_3(self, msg=LTLPlan()):
        ...
        if not (self.check_timestamp and (msg.header.stamp.to_sec() == self.prev_received_timestamp_3.to_sec())):
            ...
            rospy.logwarn("LTL Global Planner: not updating with received trace, timestamp identical to previously received message timestamp at time %f" %self.prev_received_timestamp_3.to_sec())
